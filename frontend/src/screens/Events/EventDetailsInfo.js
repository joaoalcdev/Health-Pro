...
import { updateEventAgreement } from "../../api/EventsAPI";
...
function EventDetailsInfo({ data, onStatus }) {
  ...
  useEffect(() => {
    ...
    setAgreement(agreements.agreement[data.agreementId - 1]);
  ...
  const handleUpdateAgreement = async () => {
    ...
    const response = await updateEventAgreement(agreementData, data.eventInstanceId);
      ...
      <div className="flex flex-col gap-6 ">
        ...
        <body className="flex flex-col gap-4">
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="flex w-full flex-col gap-1">
              ...
              <p className="text-black text-md font-semibold">{eventTypes[data.eventType].name}</p>
            </div>
            <div className="flex w-full flex-col gap-1">
        ...
        <footer>
          ...
          <div className="flex p-2 text-subMain justify-end items-center">
            ...
            Check-in realizado por {data.checkInName || 'Mateus Gondim'} em {data.checkInDate || new Date().toLocaleString()}
          </div>
        </footer>
      </div>
    </>
