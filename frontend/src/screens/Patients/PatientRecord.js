...
function PatientRecord({ data }) {
  ...
  const fetch = async () => {
    setLoading(true);
    // fetch data
    const response = await getPatientRecords(data.id, `01-${monthRange.getMonth() + 1}-${monthRange.getFullYear()}`);
  ...
  const exportFrequency = async () => {
    ...
    const res = await getPatientRecordsExport(data.id, `01-${monthRange.getMonth() + 1}-${monthRange.getFullYear()}`, data.patientFullName);
              ...
              <div key={data.eventInstanceId} className='grid grid-cols-1 bg-dry rounded-xl border-[1px] border-border p-4 gap-2'>
                ...
                >
                  ...
                  <div className="col-span-12 md:col-span-7 flex flex-col gap-2">
                    ...
                    <p className="text-md text-black ">{data.professionalFirstName} {data.professionalLastName} </p>
                  </div>
